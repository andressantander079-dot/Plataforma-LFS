...
export default function ArbitroGlobalCatchAll() {
  ...
  const handleGoBack = () => {
    if (parentModule) {
      router.push(`/arbitro/${parentModule}`);
    } else {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl mx-auto mt-12 text-center items-center p-4">
      <div className="w-16 h-16 bg-[#F97316]/10 text-[#F97316] rounded-full flex items-center justify-center shadow-inner">
        <AlertCircle className="w-8 h-8" />
      </div>

      <div>
        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Módulo del Árbitro - Redirección</span>
        ...
        <p className="text-slate-500 text-xs mt-2 max-w-md mx-auto">
          Esta sección se encuentra enlazada y configurada en el panel del árbitro. Serás redirigido al módulo principal correspondiente para continuar con las operaciones.
        </p>
      </div>
